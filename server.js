// Server.js — GroFriends (UI embebida, temas, amigos mejorados, perfiles y DMs)
...
if (!hasCol("users","locale")) db.exec(`ALTER TABLE users ADD COLUMN locale TEXT`);
if (!hasCol("users","theme")) db.exec(`ALTER TABLE users ADD COLUMN theme TEXT`);
if (!hasCol("users","photo_base64")) db.exec(`ALTER TABLE users ADD COLUMN photo_base64 TEXT`);
...
const qInsertSession = db.prepare(`INSERT INTO sessions (user_id, token, created_at) VALUES (@user_id, @token, @created_at)`);
const qFindSession   = db.prepare(`SELECT s.token, u.id as user_id, u.email, u.name FROM sessions s JOIN users u ON u.id = s.user_id WHERE s.token=?`);
...
function authUserFromRequest(req) {
  ...
  return row ? { id: row.user_id, email: row.email, name: row.name, token } : null;
}
  ...
  <div class="card grid cols-2">
    ...
    <div>
      ...
      <ul class="list">
        ...
        <li class="item"><span>Social</span><span class="badge">Feed</span></li>
        <li class="item"><span>Amigos + Chat</span><span class="badge">DMs</span></li>
        <li class="item"><span>Metas</span><span class="badge">Goals</span></li>
      </ul>
    </div>
  </div>
</div>
...
function activate(tab){
  ...
  if(tab==="dms") { loadDMFriends(); startDMRefresh(); }
...
async function refreshFeed(reset=false){
  ...
  for(const p of j.posts){
    ...
    const left = document.createElement("div"); left.append(strong, content, meta);
    li.append(left);
    ul.append(li);
  }
...
$("#postForm").addEventListener("submit", async (e)=>{
  ...
  $("#postText").value=""; refreshFeed(true);
});
...
async function loadFriends(){
  ...
  // incoming
  if(!parts.incoming.length){
    const li = document.createElement("li"); li.className="item";
    li.innerHTML = '<span class="muted">No tienes solicitudes.</span>'; incomingEl.append(li);
  } else {
    for(const f of parts.incoming){
      const li = document.createElement("li"); li.className="item";
  ...
  // outgoing
  if(!parts.outgoing.length){
    const li = document.createElement("li"); li.className="item";
    li.innerHTML = '<span class="muted">No has enviado invitaciones.</span>'; outgoingEl.append(li);
  } else {
    for(const f of parts.outgoing){
      const li = document.createElement("li"); li.className="item";
      li.innerHTML = '<div><strong>'+ (f.friend_name||f.friend_email) +'</strong><div class="muted">'+f.friend_email+'</div></div>';
  ...
  // accepted
  if(!parts.accepted.length){
    const li = document.createElement("li"); li.className="item";
    li.innerHTML = '<span class="muted">Aún no tienes amigos.</span>'; listEl.append(li);
  } else {
    for(const f of parts.accepted){
      const li = document.createElement("li"); li.className="item";
      li.innerHTML = '<div><strong>'+ (f.friend_name||f.friend_email) +'</strong><div class="muted">'+f.friend_email+'</div></div>';
      const bar = document.createElement("div"); bar.className="btnbar";
...
// DMs
let currentDM = null, dmOffset = 0, dmRefreshTimer = null;
function stopDMRefresh(){ if(dmRefreshTimer){ clearInterval(dmRefreshTimer); dmRefreshTimer=null; } }
function startDMRefresh(){ stopDMRefresh(); dmRefreshTimer = setInterval(()=>loadDMs(false,true), 4000); }
...
async function selectDMFriend(id, name){
  ...
  await loadDMs(true,false);
  startDMRefresh();
}
async function loadDMs(reset=false, silent=false){
  if(!currentDM) return;
...
$("#dmForm").addEventListener("submit", async (e)=>{
  ...
  $("#dmText").value=""; loadDMs(true,false);
});
$("#dmLoadMore").addEventListener("click", ()=>loadDMs(false,false));
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && path === "/ui") {
    ...
    return res.end(PAGE_LOGIN);
  }
  if (req.method === "GET" && path === "/ui/register") {
    res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
    return res.end(PAGE_REGISTER);
  }
  if (req.method === "GET" && path === "/app") {
    res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
    return res.end(PAGE_APP);
  }
  ...
  if (req.method === "POST" && path === "/auth/register") {
    try {
      ...
      qInsertUser.run({ email, password_hash, name, created_at: new Date().toISOString() });
  ...
  if (req.method === "POST" && path === "/auth/login") {
    try {
      ...
      qInsertSession.run({ user_id: u.id, token, created_at: new Date().toISOString() });
      return sendJson(res, 200, { ok:true, token });
    } catch { return sendJson(res, 400, { ok:false, error:"Bad request" }); }
  }

  if (req.method === "GET" && path === "/auth/me") {
    const u = authUserFromRequest(req); if (!u) return sendJson(res, 401, { ok:false, error:"Unauthorized (no/invalid token)" });
  ...
  if (req.method === "POST" && path === "/items") {
    ...
    try {
      ...
      qInsertItem.run({ user_id: u.id, title, qty, note, done: 0, created_at: new Date().toISOString() });
      return sendJson(res, 201, { ok:true });
    } catch { return sendJson(res, 400, { ok:false, error:"Bad request" }); }
  }
  ...
  if (req.method === "POST" && path === "/friends/invite") {
    ...
    try {
      ...
      qInsertFriendship.run({ user_a: a, user_b: b2, status: "pending", requested_by: u.id, created_at: new Date().toISOString() });
      return sendJson(res, 201, { ok:true });
    } catch { return sendJson(res, 400, { ok:false, error:"Bad request" }); }
  }
  ...
  if (req.method === "POST" && path === "/goals") {
    ...
    try {
      ...
      const info = qInsertGoal.run({ user_id:u.id, title, target_date, is_public:0, created_at: now });
  ...
  if (req.method === "POST" && path.startsWith("/goals/") && path.endsWith("/publish")) {
    ...
    if (g) qInsertFeed.run({ user_id: u.id, goal_id: id, content: "New goal: " + g.title, created_at: new Date().toISOString() });
    return sendJson(res, 200, { ok:true });
  }
  if (req.method === "GET" && path === "/feed") {
    ...
    const rows = qListFeed.all(limit, offset).map(r => ({ id:r.id, user_id:r.user_id, content:r.content, created_at:r.created_at, name:r.name, email:r.email }));
  ...
  if (req.method === "POST" && path === "/feed") {
    ...
    try {
      ...
      qInsertFeed.run({ user_id:u.id, goal_id: null, content, created_at: new Date().toISOString() });
      return sendJson(res, 201, { ok:true });
    } catch { return sendJson(res, 400, { ok:false, error:"Bad request" }); }
  }
  ...
  if (req.method === "GET" && path === "/dm") {
    ...
    const rows = qListDM.all({ me:u.id, friend:friend_id, limit, offset }).map(m => ({ id:m.id, text:m.text, created_at:m.created_at, mine:m.sender_id===u.id }));
  ...
  if (req.method === "POST" && path === "/dm/send") {
    ...
    try {
      ...
      if (!f || f.status!=="accepted") return sendJson(res, 403, { ok:false, error:"Not friends" });
      qInsertDM.run({ sender_id:u.id, receiver_id:friend_id, text, created_at:new Date().toISOString() });
      return sendJson(res, 201, { ok:true });
    } catch { return sendJson(res, 400, { ok:false, error:"Bad request" }); }
  }
